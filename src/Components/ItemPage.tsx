...
const ItemInfoContainer = ({ item }: ItemProp) => {
  return (
    <section className="col-span-3 grid grid-rows-[min-content] grid-cols-3 w-full max-h-min">
  ...
  return (
    <section>
      <h2 className="text-3xl font-bold">
        ESRB Rating:
        {item.esrb_rating ? item.esrb_rating : ""}
      </h2>
    </section>
  );
};
...
const MetaContainer = ({ item }: ItemProp) => {
  return (
    <section className="col-span-2 row-span-2  grid  grid-cols-3 grid-rows-3 max-h-[30vh] items-center">
      <Platforms item={item} />
  ...
  return (
    <section>
      {item.genres.map((genre) => (
        <div key={genre.id} className="rounded font-bold">
          {genre.name}
        </div>
      ))}
    </section>
...
const Platforms = ({ item }: ItemProp) => {
  ...
  }; //HUGE thing, keep collapsed

  return (
    <section className="grid grid-flow-col grid-rows-1 ">
      {item.platforms.map((plat) => (
        <div key={plat.platform.id}>{svgPaths[plat.platform.slug]}</div>
      ))}
    </section>
  );
};
  ...
  return (
    <section className="col-span-3 ">
      ...
      {paragraphs.map((par, index) => (
        <p key={index} className="p-6 text-2xl">
          {par}
        </p>
      ))}
    </section>
...
const DLCContainer = ({ additions, page }) => {
  ...
  return <ImgContainer imgs={imgs} />;
};

const SeriesContainer = ({ series, page }) => {
...
/* <figure className="border-2 border-black border-solid col-span-2 row-span-1 flex flex-grow-0 flex-shrink-0  overflow-hidden relative max-h-[80vh]">
      {additions.additions.results.map((add) => (
        <img
          key={add.id}
          src={add.background_image}
          alt=""
          title={add.name}
